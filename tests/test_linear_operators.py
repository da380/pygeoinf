...
class TestDeflatedDiagonal:
    """
    Tests for the deflated_diagonal method on LinearOperators.
    Because the autouse fixture sets a random seed, these stochastic
    tests are deterministic across runs.
    """

    def test_deflated_diagonal_pure_low_rank(self):
        """Tests that rank>0, size_estimate=0 extracts the exact diagonal for a low-rank matrix."""
        domain = inf.EuclideanSpace(6)
        # Create an exact rank-2 matrix
        A = np.random.randn(6, 2)
        B = np.random.randn(2, 6)
        M = A @ B
        op = inf.LinearOperator.from_matrix(domain, domain, M, galerkin=False)

        # Extract using exact low-rank logic
        diag_est = op.deflated_diagonal(2, 0, galerkin=False)
    ...
    def test_deflated_diagonal_pure_hutchinson(self):
        ...
        domain = inf.EuclideanSpace(5)
        M = np.random.randn(5, 5)
        op = inf.LinearOperator.from_matrix(domain, domain, M, galerkin=True)

        # FIX: Explicitly set max_samples so it isn't clipped to dimension 5
        diag_est = op.deflated_diagonal(
            0, 5000, method="fixed", max_samples=5000, galerkin=True
        )

        # Generous tolerance due to stochastic nature
        assert np.allclose(diag_est, np.diag(M), rtol=0.1, atol=0.1)

    def test_deflated_diagonal_mixed_asymmetric(self):
        """Tests the full deflated method on a non-symmetric matrix to verify the SVD logic."""
        domain = inf.EuclideanSpace(6)
        M = np.random.randn(6, 6)  # Not symmetric, not positive definite
        op = inf.LinearOperator.from_matrix(domain, domain, M, galerkin=False)

        # FIX: Explicitly set max_samples so it isn't clipped to dimension 6
        diag_est = op.deflated_diagonal(
            3, 2000, method="fixed", max_samples=2000, galerkin=False
        )
