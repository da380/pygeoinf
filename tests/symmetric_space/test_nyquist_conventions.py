...
2. Point evaluation (`dirac`, and the NUFFT matrix-free operator on the
   torus) reproduces a function's grid values exactly, including functions
   with Nyquist content.
...
class TestTorusNyquist:
    ...
    def test_dirac_reproduces_grid_values(self, matrix_free):
        ...
        op = space.point_evaluation_operator(points, matrix_free=matrix_free)
        np.testing.assert_allclose(op(u), expected, rtol=1e-10, atol=1e-10)

    def test_matrix_free_point_evaluation_matches_dense(self):
        # The NUFFT operator must share the dense operator's Nyquist
        # convention in both the forward map and its adjoint.
        space = TorusSobolev(KMAX, 2.0, 0.1)
        rng = np.random.default_rng(45)
        points = [tuple(p) for p in rng.uniform(0.0, 2 * np.pi, size=(20, 2))]
        dense = space.point_evaluation_operator(points)
        nufft = space.point_evaluation_operator(points, matrix_free=True)

        u = space.from_components(rng.standard_normal(space.dim))
        np.testing.assert_allclose(nufft(u), dense(u), rtol=1e-9, atol=1e-10)

        y = rng.standard_normal(len(points))
        np.testing.assert_allclose(
            space.to_components(nufft.adjoint(y)),
            space.to_components(dense.adjoint(y)),
            rtol=1e-8,
            atol=1e-10,
        )
