...
def model_measure(model_space: HilbertSpace) -> GaussianMeasure:
    ...
    return GaussianMeasure.from_covariance_matrix(
        model_space, cov_matrix, expectation=mean
    )


@pytest.fixture
def forward_operator(
    model_space: HilbertSpace, data_space: HilbertSpace
) -> LinearOperator:
...
class TestEmpiricalDataErrorMeasure:
    ...
    def test_returns_gaussian_measure(
        self, model_measure: GaussianMeasure, forward_operator: LinearOperator
    ):
    ...
    def test_mean_centering_behavior(
        self, model_measure: GaussianMeasure, forward_operator: LinearOperator
    ):
        ...
        assert np.allclose(
            result_mean, expected_zero_mean, atol=0.1
    ...
    def test_scale_factor_effect(
        ...
        assert np.allclose(
            cov_scaled, expected_scaled_cov, rtol=1e-12
    ...
    def test_empirical_vs_analytical_mean_behavior(
        self, model_measure: GaussianMeasure, forward_operator: LinearOperator
    ):
        ...
        assert np.allclose(
            result_mean, expected_zero_mean, atol=0.2
    ...
    def test_consistent_results_with_fixed_seed(
        self, model_measure: GaussianMeasure, forward_operator: LinearOperator
    ):
    ...
    def test_approximates_analytical_covariance(
        self, model_space: HilbertSpace, data_space: HilbertSpace
    ):
        ...
        assert np.allclose(
            empirical_cov, analytical_cov, rtol=0.3, atol=0.3
