...
**Oracle optimisations** (implemented 2026-03-10):
...
- `value_and_subgradient` calls `value_and_support_point` (fused API) on each support function, obtaining `(scalar_value, support_point)` in one call. When the support point is available the scalar value is already obtained from the fused call, eliminating the redundant separate `_mapping` evaluation. Fallback path (support point unavailable) is unchanged.
