...
- `SubspaceSlicePlotter`
  ...
  - Uses exact slicing for `PolyhedralSet` (affine HalfspaceIntersection), `Ball`, and `Ellipsoid` (quadratic pullback via Cholesky-factored shape operator); falls back to sampled membership masks for other subsets.
...
- [tests/test_plot_exact_quadratic_slices.py](/home/adrian/PhD/Inferences/pygeoinf/tests/test_plot_exact_quadratic_slices.py)
  - 25 tests covering exact 1D/2D/3D Ball and Ellipsoid slice rendering, backend dispatch, and the bypass of membership sampling.
- [tests/symmetric_space/test_sphere_cap_integrals.py](/home/adrian/PhD/Inferences/pygeoinf/tests/symmetric_space/test_sphere_cap_integrals.py)
  - Spherical-cap average integration tests on `Sphere`, verifying normalisation, constant-field consistency, and cap-operator shape.
...
- The weighted-chi-square auto-selection follow-up adds 16 tests in [tests/test_quadratic_form_quantile.py](/home/adrian/PhD/Inferences/pygeoinf/tests/test_quadratic_form_quantile.py), bringing that module to 58 tests.
- The exact Ball/Ellipsoid plotter adds 25 tests in `test_plot_exact_quadratic_slices.py`. The sphere-cap integration work adds tests in `tests/symmetric_space/test_sphere_cap_integrals.py`. Current validated pygeoinf suite count: **941 passing tests** (1 xfailed).