...
model_space.plot_geodesic_network(paths, ax1)
fig1.colorbar(im1)

# Plot the true model
fig2, ax2, im2 = model_space.plot(model_posterior.expectation)
model_space.plot_geodesic_network(paths, ax2)
fig2.colorbar(im2)

# Plot the pointwise std
model_posterior_std = model_posterior.sample_pointwise_std(500)
fig3, ax3, im3 = model_space.plot(model_posterior_std)
model_space.plot_geodesic_network(paths, ax3)
fig3.colorbar(im3)
