...
class LinearBayesianInversion(LinearInversion):
    ...
    def log_det_evidence_term(
        ...
        *,
        ...
        if log_det_noise_cov is not None:
            ...
            lu_factor = splinalg.splu(R.sparse_array.tocsc())
        ...
        if R_measure.inverse_covariance_set:
            ...
            R_inv = noise_solver(R)

        # =====================================================================
        # 2. Prior Covariance Factorization
        # =====================================================================
        if Q_measure.covariance_factor_set:
            L = Q_measure.covariance_factor
        else:
            lr_prior = Q_measure.low_rank_approximation(
                rank,
                method=randomized_method,
                max_rank=max_rank,
                power=power,
                rtol=rtol,
            ...
            L = lr_prior.covariance_factor

        # =====================================================================
        # 3. Symmetrized Low-Rank Approximation
        # =====================================================================
        ...
        log_det_I_H = float(np.sum(np.log(1.0 + eig_approx.eigenvalues)))

        return R_log_det + log_det_I_H
    ...
    def log_marginal_likelihood(
        ...
        *,
        ...
        """
        Estimates the log marginal likelihood (Bayesian Evidence) ln p(d).
        ...
        Args:
            ...
            log_det_noise_cov: The pre-computed log determinant of the noise covariance.
                               Required if R is a generic matrix-free operator.
            preconditioner: An optional preconditioner for the normal equations solver.
            rank: The target rank for the randomized eigendecomposition.
            randomized_method: Algorithm for randomized SVD ('variable' or 'fixed').
            max_rank: Hard limit on the rank for the 'variable' method.
            power: Number of power iterations to improve spectral accuracy.
        ...
        log_det = self.log_det_evidence_term(
            ...
            log_det_noise_cov=log_det_noise_cov,
            rank=rank,
        ...
        log_evidence = -0.5 * mahalanobis - 0.5 * log_det - 0.5 * M * np.log(2 * np.pi)
