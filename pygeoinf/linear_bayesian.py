...
class LinearBayesianInversion(LinearInversion):
    ...
    def log_evidence(
        ...
        *,
        ...
        Formalism Optimization:
        -----------------------
        If 'data_space' is selected, the log-determinant ln|N_d| is estimated directly
        via Stochastic Lanczos Quadrature (SLQ), requiring thousands of evaluations
        of the forward operator A.

        If 'model_space' is selected, the method exploits Sylvester's determinant
        identity to completely bypass forward operator evaluations in the trace estimator:
            ln|N_d| = ln|N_m| + ln|Q| + ln|R|
        This decomposes the massive data-space determinant into three smaller, highly
        efficient SLQ evaluations on the model space, prior covariance, and noise covariance.
