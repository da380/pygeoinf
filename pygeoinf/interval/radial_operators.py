...
class RadialLaplacianEigenvalueProvider(EigenvalueProvider):
    ...
    def _compute_regularity_neumann_eigenvalue(
        ...
        # tan(kR) = kR
        F = lambda k: k * R
        k_root = self._root_between_tan_poles(F, R, index)
        return k_root ** 2

    def _root_between_tan_poles(
            self, F, L: float, n: int, eps: float = 1e-8
    ) -> float:
        """
        Solve tan(k L) = F(k) for the root in the interval
        ((n+1/2)π/L, (n+3/2)π/L), nudged by 'eps' away from poles.

        All transcendental eigenvalue equations for the radial Laplacian
        can be written as tan(kL) = F(k), which has poles at (n+1/2)π/L.
        Between any two consecutive poles, there is exactly one root.

        Args:
            F: Right-hand side function F(k)
            L: Domain length
            n: Pole interval index (root is between n-th and (n+1)-th pole)
        ...
        left = ((n + 0.5) * np.pi) / L + eps
        right = ((n + 1.5) * np.pi) / L - eps

        def G(k):  # equation = 0
            return np.tan(k * L) - F(k)

        # Brent requires opposite signs; near poles this is effectively guaranteed,
        # but we still add a tiny widening fallback to be safe:
        try:
            return brentq(G, left, right)
        except ValueError:
            widen = 10 * eps
            return brentq(G, left + widen, right - widen)
    ...
    def _compute_dn_eigenvalue(
        ...
        # tan(kL) = kb
        F = lambda k: k * b
        k_root = self._root_between_tan_poles(F, L, index)
        return k_root ** 2
    ...
    def _compute_nn_eigenvalue(
        ...
        k_root = self._root_between_tan_poles(F, L, index - 1)
        return k_root ** 2
