"""
Unit tests for FEM solvers.

Tests both GeneralFEMSolver and FEMSolver classes including the analytical
stiffness matrix optimization for hat functions.
"""
...
from pygeoinf.interval.fem_solvers import GeneralFEMSolver, FEMSolver
...
class TestGeneralFEMSolver(unittest.TestCase):
    ...
    def test_analytical_vs_legacy_fem_solver_match(self):
        """Test analytical stiffness matrix matches legacy FEM solver."""
        # Create GeneralFEMSolver with hat functions
        l2_space = L2Space(self.n_dofs, self.domain, basis_type='hat_homogeneous')
        general_solver = GeneralFEMSolver(l2_space, self.bc_homogeneous)
        general_solver.setup()

        # Create legacy FEMSolver
        legacy_solver = FEMSolver(self.domain, self.n_dofs, self.bc_homogeneous)
        legacy_solver.setup()

        # Get stiffness matrices
        K_general = general_solver.get_stiffness_matrix()
        K_legacy = legacy_solver._assemble_stiffness_matrix()

        # They should match exactly
        np.testing.assert_allclose(K_general, K_legacy, rtol=1e-15)
