...
class VectorSpaceDirectSum(VectorSpace):
    ...
    user provides a list of VectorSpaces.
    """
    ...
    def subspace_projection(self, i):
        ...
        return LinearOperator(
            ...
            dual_mapping=lambda ypi: self._dual_subspace_projection_mapping(i, ypi),
        )
    ...
    def subspace_inclusion(self, i):
        ...
        return LinearOperator(
            ...
            dual_mapping=lambda xp: self._dual_subspace_inclusion_mapping(i, xp),
        )
    ...
    def _dual_subspace_projection_mapping(self, i, ypi):
    ...
    def _subspace_inclusion_mapping(self, i, x):
        ...
        xs.append(x)
        for j in range(i + 1, self.number_of_spaces):
            xs.append(self._spaces[j].zero)
        return xs

    def _dual_subspace_inclusion_mapping(self, i, xp):
        # Implementation of dual of the inclusion mapping from the ith space.
        return LinearForm(
            self._spaces[i],
            mapping=lambda xi: xp(self._subspace_inclusion_mapping(i, xi)),
        )
...
class LinearOpeatorDirectSum(LinearOperator):
    """
    Class for the direct sum of a list of operators with a common domain.
    """
    ...
    def __init__(self, operators):
        ...
        if all([operator.hilbert_operator for operator in operators]):
            codomain = HilbertSpaceDirectSum(
                [operator.codomain for operator in operators]
            )
        else:
            codomain = VectorSpaceDirectSum(
                [operator.codomain for operator in operators]
            )

        super().__init__(
            domain, codomain, lambda x: [operator(x) for operator in operators]
        )
...
class HilbertSpaceDirectSum(HilbertSpace):
    """
    Class for the direct sum of Hilbert spaces. To define an instance, the
    user provides a list of HilbertSpaces.
    """
    ...
    def __init__(self, spaces):
        ...
        self._spaces = spaces
        dim = sum([space.dim for space in spaces])
        vector_space = VectorSpaceDirectSum(spaces)
        super().__init__(
            dim,
            vector_space.to_components,
            vector_space.from_components,
            self._inner_product,
            self._to_dual,
            self._from_dual,
            add=vector_space.add,
            subtract=vector_space.subtract,
            multiply=vector_space.multiply,
            axpy=vector_space.axpy,
            copy=vector_space.copy,
        )

    def _inner_product(self, x1s, x2s):
        # Local implementation of the inner product.
        return sum(
            [X.inner_product(x1, x2) for X, x1, x2 in zip(self._spaces, x1s, x2s)]
        )
