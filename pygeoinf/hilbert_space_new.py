"""
Defines foundational abstractions for working with Hilbert spaces.
...
- `MassWeightedHilbertModule`: A mass-weighted space that also supports vector
  multiplication.
"""
...
class HilbertSpace(ABC):
    ...
    NumPy arrays). Subclasses must implement all abstract methods.
    """
...
class DualHilbertSpace(HilbertSpace):
    ...
    by the `LinearForm` class. This class provides a `HilbertSpace` interface
    for such elements.
    """
...
class MassWeightedHilbertSpace(HilbertSpace):
    ...
    This class wraps an existing `HilbertSpace` (X) and defines a new inner
    product for a space (Y) as: `(u, v)_Y = (M u, v)_X`, where `M` is a
    self-adjoint, positive-definite mass operator on X.
    """
