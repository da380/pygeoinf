...
def empirical_data_error_measure(
    model_measure, forward_operator, n_samples=10, scale_factor=1.0
):
    """
    ...
    data_samples = model_measure.affine_mapping(operator=forward_operator).samples(
        n_samples
    )
    ...
    zeroed_samples = [
        data_space.multiply(scale_factor, data_space.subtract(data_sample, mean))
