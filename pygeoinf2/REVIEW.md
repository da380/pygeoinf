...
**Phase 3 outcome (2026-08-28).** All Must items done, and Should-11/12/14/15/16/17/18/19/20/21/23 with them; Consider-30 became `tests/test_space_parity.py`, which runs every geometric method on all six geometries and is what caught the padded-box coordinate bug. Three notes worth keeping:

- **Y Should-13 is largely already done, and its stated benefit is misdiagnosed.** The forward round trip it names is gone: `from_formal_adjoint` moves components directly when both sides are coordinate spaces, so `multiplication_operator` costs 2 transforms forward, which is the floor for a pointwise multiply on a spectral space. The adjoint costs 6, and 4 of those are structural -- `operator.adjoint` takes a *vector*, and the array it must be handed is a reweighted one, so it has to be synthesised. The `flexural_operator` figure the item quotes (58, measured here as 50 forward and 54 adjoint) is not lift overhead at all: the lift happens once at the top and accounts for exactly the 4-transform difference between the two. The other 50 are the formula's own round trips to the grid -- roughly 25 `laplacian`, `multiply` and `gradient_dot_product` calls, each analysing and synthesising. Closing the gap to v1's 30 means fusing the pointwise work on the grid, which is a real optimisation but a different and larger one. Left undone deliberately.
- **Y Should-14 was already done.** `_path_nodes` carries v1's `ceil(2 * arc / length_scale)` heuristic with a floor of two, and the integral/average split is the explicit rename the item asked for as the alternative.
- **An API question surfaced rather than decided.** `power_spectrum` and `power_measure` agree on a Lebesgue space and differ on a Sobolev one by the Sobolev symbol, because `invariant_measure`'s eigenvalues are the covariance *operator's* in that space's metric. Whether `power_measure` should mean the `H^s` spectrum it currently means or the `L2` one a modeller more often writes down is recorded in its docstring and left open.
