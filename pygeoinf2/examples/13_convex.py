...
smooth = SquaredDistance(X, center=center)
...
print(
    ...
    X.norm(X.subtract(result.minimizer, penalty.prox(center, 1.0))) < 1e-8,
)
print()
...
print(f"with ||x|| <= 0.2 : ||x*|| = {X.norm(constrained.minimizer):.6f}")
print()
...
shifted = ball + SupportFunction.of_point(X, center)
y = X.random(rng=rng)
print(
    "h_ball(y)            :",
    round(ball(y), 6),
