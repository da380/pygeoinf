...
| `MassWeightedHilbertSpace` | Ported | `algebra.spaces.MassWeightedSpace`: `(x, y)_V == (M x, y)_base`, deriving `M^-1` from a solver rather than making the caller supply it; a coordinate space when the base is one, so direct solvers and `matrix()` accept it as v1's delegation allowed (DESIGN §83). The row used to say "Subsumed" into any `CoordinateSpace` with a non-trivial Gram matrix; that is a different construction — a Gram matrix weights the *components*, a mass operator weights *another inner product* on the same vectors, and only the second works over a backend with no component map (§3.2, §15.1). `LinearOperator.from_formal_adjoint` exists for it | |
| `MassWeightedHilbertModule` | Ported | `MassWeightedSpace(base, mass)` picks its variant by the base: over a module it is a `HilbertModule` with `multiply` and `sqrt` delegated, over a coordinate space it is a `CoordinateSpace` with Gram `G_base M_c`, over both it is both -- v1's module (DESIGN §83) | |
...
| `plot.py` — `SubspaceSlicePlotter`, `plot_slice` | Ported | `plotting.plot_set(subset, subspace=, bounds=, route=)`: one and two dimensions, matplotlib only, the route chosen by capability -- the support polygon, the level contour, or a membership raster -- so v1's polyhedral and quadratic paths fall out without a class check; slices through `subspace=`, projections through `push_forward`. 3-D, voxels and plotly left for later (David, 2026-09-17: the compact port; Mag can expand it). DESIGN §80 | |
...
| `CorrelatedInvariantGaussianMeasure.marginal`, `.cross_covariance`, `.spectral_correlations` | Ported | `measure.marginal(i)`, `measure.cross_covariance(i, j)` and `space.spectral_correlations(measure)`. The first two are general -- any Gaussian on a direct sum has them -- and read the block straight off, so a diagonal marginal keeps its eigenvalues and its sampling factor. The only route before was slicing the covariance's dense component matrix | |
...
| `posterior_expectation_operator` | Ported | `GaussianEstimator.expectation_operator` (DESIGN §79: David prefers expectation to mean) | |
...
| `backends.mfem` essential boundary conditions | `MfemSpace(elements, essential_dofs=...)` and `essential_dofs_of`. A homogeneous Dirichlet condition is the *subspace* of functions vanishing on the boundary, so the Gram matrix is the mass matrix's free block and everything else applies unchanged (DESIGN §33.1) | |
| `backends.mfem.solver_from_bilinear_form`, `operator_from_linear_forms` | MFEM's own solver solves MFEM's own system and comes back as a `LinearOperator`; observations are stacked load forms. The division of labour is MFEM for the discretisation, this library for the inverse problem (DESIGN §33.4) | |
| `backends.mfem.matern_measure`, `white_noise_load` | The Lindgren SPDE method for Gaussian random fields, wrapped from MFEM's `miniapps/spde` rather than reimplemented: its bilinear forms, its solves, its white-noise integrator, and a `GaussianMeasure` with covariance, factor and precision (DESIGN §34) | |
