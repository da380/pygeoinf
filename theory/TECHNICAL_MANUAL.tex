...
All of the objects in this chapter are defined between infinite-dimensional
spaces. Nevertheless, numerical algorithms require finite arrays. A
\emph{matrix representation} is nothing more than the coordinate description of
the restriction of $G$ to a finite-dimensional trial space and a finite-
dimensional test space.

The key point for \texttt{pygeoinf} is that there are (at least) two natural
matrix \emph{views} of the same operator, and they differ in what the
\texttt{rmatvec} operation means:
\begin{enumerate}
  \item \textbf{Component view} (\texttt{galerkin=False}): \texttt{matvec} maps
    \emph{primal components} to \emph{primal components}, while \texttt{rmatvec}
    represents the \emph{Banach dual} operator $G'$ and therefore acts on
    \emph{dual components}.
  \item \textbf{Galerkin view} (\texttt{galerkin=True}): \texttt{matvec} maps
    \emph{primal components} to \emph{dual components} via a Riesz map in the
    codomain, and \texttt{rmatvec} represents the \emph{Hilbert adjoint}
    operator $G^*$.
\end{enumerate}

Both views are valid; which one you want depends on whether your algorithm is
formulated in terms of duality pairings ($G'$) or inner products ($G^*$).

\paragraph{Coordinates are not geometry.}
The maps \texttt{to\_components/from\_components} are \emph{coordinate maps}.
They depend on a chosen basis and are not assumed to be isometries. In
particular, do \emph{not} assume the basis is orthonormal; the inner products
live separately, encoded by the Gram (``mass'') matrices and Riesz maps.

\subsection{The standard (component) matrix}
\label{ssec:standard-matrix}

Let $\modelspace$ and $\dataspace$ be Hilbert spaces, and choose finite-
dimensional subspaces
\(
  \modelspace_N = \spn\{\phi_1,\ldots,\phi_N\} \subset \modelspace
\)
and
\(
  \dataspace_M = \spn\{\psi_1,\ldots,\psi_M\} \subset \dataspace.
\)
Write
\(\pi_{\modelspace}:\mathbb{R}^N\to\modelspace_N\) for synthesis
(``from components'') and
\(\Pi_{\dataspace}:\dataspace_M\to\mathbb{R}^M\) for analysis
(``to components'').

The \emph{standard component matrix} of $G$ (with respect to these coordinate
maps) is
\begin{equation}
  \label{eq:standard-matrix-def}
  \Gd \;:=\; \Pi_{\dataspace}\,\circ\, G\,\circ\,\pi_{\modelspace}
  \;\in\; \mathbb{R}^{M\times N},
\end{equation}
so that for a coefficient vector $\mathbf{c}\in\mathbb{R}^N$ we have
\(
  \Gd\,\mathbf{c} = \Pi_{\dataspace}\bigl(G(\pi_{\modelspace}\mathbf{c})\bigr),
\)
the \emph{primal components} of the predicted data.

In this component view, \texttt{pygeoinf} uses
\texttt{G.matrix(galerkin=False)} to build a matrix(-free) operator with
\texttt{matvec} implementing \eqref{eq:standard-matrix-def}.

\paragraph{What \texttt{rmatvec} means when \texttt{galerkin=False}.}
The reverse multiplication does \emph{not} represent the Hilbert adjoint.
Instead, it represents the \emph{Banach dual} $G'$:
\begin{equation}
  \label{eq:standard-rmatvec-dual}
  \texttt{rmatvec}:\; \mathbb{R}^M \;\longrightarrow\; \mathbb{R}^N
  \quad\text{corresponds to}\quad
  G' : \dataspace' \to \modelspace'.
\end{equation}
Concretely, its input is interpreted as \emph{dual} components of some
$d'\in\dataspace'$ and its output is the \emph{dual} component vector of
$G'(d')\in\modelspace'$. This is the correct object when your algorithm is
written in terms of dual pairings (for example, building linear functionals or
working directly in $\dataspace'$).

\subsection{The Galerkin matrix}
\label{ssec:galerkin-matrix}

When $\modelspace$ and $\dataspace$ are Hilbert spaces, we can convert primal
vectors to dual vectors using the Riesz maps. Define the Gram (mass) matrix of
the test space basis by
\begin{equation}
  \label{eq:mass-matrix-data}
  \mass_{\dataspace} \in \mathbb{R}^{M\times M},
  \qquad
  (\mass_{\dataspace})_{ij} := \ipD{\psi_j}{\psi_i}.
\end{equation}
This matrix is precisely the coordinate representation of the map
$\Riesz_{\dataspace}^{-1}:\dataspace_M\to\dataspace_M'$: for
$d\in\dataspace_M$ with components $\mathbf{b}=\Pi_{\dataspace}(d)$,
the dual element $\Riesz_{\dataspace}^{-1}d\in\dataspace'$ has dual components
$\mass_{\dataspace}\,\mathbf{b}$.

The \emph{Galerkin matrix} is then the coordinate representation of
$\Riesz_{\dataspace}^{-1}\circ G$:
\begin{equation}
  \label{eq:galerkin-matrix-def}
  \GdGal
  \;:=\;
  \Pi_{\dataspace'}\,\circ\, \Riesz_{\dataspace}^{-1}\,\circ\, G\,\circ\,\pi_{\modelspace}
  \;\in\; \mathbb{R}^{M\times N}.
\end{equation}
In the common situation where $\Pi_{\dataspace'}$ is the dual coordinate map
associated with the basis $\{\psi_j\}$, we have the concrete relation
\begin{equation}
  \label{eq:galerkin-vs-standard}
  \GdGal = \mass_{\dataspace}\,\Gd.
\end{equation}
So \texttt{galerkin=True} does \emph{not} merely change a convention; it
changes which space the output coordinates live in (primal vs dual).

\paragraph{What \texttt{rmatvec} means when \texttt{galerkin=True}.}
In this view the reverse multiplication corresponds to the \emph{Hilbert
adjoint} $G^*$.
More precisely, with \texttt{G.matrix(galerkin=True)}, the forward operation is
\(
  \mathbf{c} \mapsto \Pi_{\dataspace'}(\Riesz_{\dataspace}^{-1}G(\pi_{\modelspace}\mathbf{c}))
\)
and the reverse operation is
\begin{equation}
  \label{eq:galerkin-rmatvec-adjoint}
  \mathbf{b} \mapsto \Pi_{\modelspace'}\bigl(\Riesz_{\modelspace}^{-1}G^*(\pi_{\dataspace}\mathbf{b})\bigr).
\end{equation}
This is the representation that makes the adjoint appear as a transpose at the
matrix level and is therefore the right choice for algorithms that depend on
symmetry/self-adjointness.

\begin{pitfallbox}
\textbf{Do not mix up the two matrix modes.}
With \texttt{galerkin=False}, \texttt{rmatvec} acts on \emph{dual} components
and represents $G'$. With \texttt{galerkin=True}, \texttt{rmatvec} acts on
\emph{primal} components and represents $G^*$. Treating one as the other leads
to silent scaling errors (missing Gram/Riesz factors) and can break algorithms.

\medskip
\textbf{Where symmetry shows up.}
If $\modelspace=\dataspace$, the discretisation uses the same trial/test space,
and $G$ is self-adjoint ($G=G^*$), then the Galerkin matrix $\GdGal$ is
\emph{symmetric} ($\GdGal=\GdGal^\top$). The standard component matrix $\Gd$ is
generally \emph{not} symmetric unless the chosen basis is orthonormal.
\end{pitfallbox}

\subsection{The discrete adjoint formula}
\label{ssec:discrete-adjoint}

Sometimes one wants a matrix that maps \emph{primal} components in the data
space to \emph{primal} components in the model space, i.e.
\begin{equation}
  \label{eq:adjoint-component-matrix-def}
  \Gadj_{\mathrm{comp}}\;:=\; \Pi_{\modelspace}\,\circ\,G^*\,\circ\,\pi_{\dataspace}
  \;\in\;\mathbb{R}^{N\times M},
\end{equation}
so that $\Pi_{\modelspace}(G^*(d))=\Gadj_{\mathrm{comp}}\,\Pi_{\dataspace}(d)$.
In non-orthonormal coordinates this is \emph{not} simply $\Gd^\top$.

Let $\mass_{\modelspace}\in\mathbb{R}^{N\times N}$ and
$\mass_{\dataspace}\in\mathbb{R}^{M\times M}$ be the Gram matrices of the
chosen bases, i.e.
\( (\mass_{\modelspace})_{ij}=\ipM{\phi_j}{\phi_i} \) and
\( (\mass_{\dataspace})_{ij}=\ipD{\psi_j}{\psi_i} \).
Then the coordinate form of the commuting relation
\eqref{eq:adjoint-from-dual} yields the standard discrete adjoint identity
\begin{equation}
  \label{eq:discrete-adjoint-formula}
  \Gadj_{\mathrm{comp}} 
  \;=\;
  \mass_{\modelspace}^{-1}\,\Gd^\top\,\mass_{\dataspace}.
\end{equation}

This formula makes explicit why the transpose is only the adjoint in Euclidean
coordinates: if $\mass_{\modelspace}=I$ and $\mass_{\dataspace}=I$ (orthonormal
bases with the standard inner product), then $\Gadj_{\mathrm{comp}}=\Gd^\top$.
In general, the Gram matrices are unavoidable.

\paragraph{Connection to \texttt{LinearOperator.matrix}.}
The component view (\texttt{galerkin=False}) returns $\Gd$ and its
\texttt{rmatvec} corresponds to $G'$ (dual-to-dual), not to
$\Gadj_{\mathrm{comp}}$. The Galerkin view (\texttt{galerkin=True}) returns a
matrix whose transpose implements the adjoint action at the level of
coefficients (via \eqref{eq:galerkin-rmatvec-adjoint}), and is the view in
which self-adjointness manifests as matrix symmetry.

\section{Worked example: \texttt{from\_matrix}, dual vs adjoint, and Galerkin matrices}
\label{sec:linear-operator-worked-example}

This chapter is written with an infinite-dimensional viewpoint: $G$ is an
operator between Hilbert spaces. The following example is therefore to be read
as a \emph{coordinate representation} used for computation (think: restricting
to a finite trial/test space and choosing bases), not as a change in the
underlying mathematical objects.

\paragraph{Set-up.}
Let the model space be Euclidean,
\(\modelspace = \mathbb{R}^2\) with the standard inner product.
Let the data space be a \emph{mass-weighted} Euclidean space built from an
underlying $H=\mathbb{R}^2$:
\begin{equation}
  \ipD{u}{v} = u^\top W v,
  \qquad
  W = \diag(2,3).
\end{equation}
The geometry is encoded by the mass/Gram operator $W$ (equivalently the Riesz
maps). In particular, identifying a primal $d\in\dataspace$ with a dual element
$d'\in\dataspace'$ introduces $W$.

Now define a tiny matrix-backed operator $G:\modelspace\to\dataspace$ via
...
V = EuclideanSpace(2)                      # model space (primal vectors are length-2 arrays)
H = EuclideanSpace(2)

W = LinearOperator.self_adjoint_from_matrix(H, np.diag([2.0, 3.0]))
W_inv = LinearOperator.self_adjoint_from_matrix(H, np.diag([0.5, 1.0/3.0]))
D = MassWeightedHilbertSpace(H, W, W_inv)  # data space with <u,v>_D = u^T W v

A = np.array([[1.0, 2.0],
              [0.0, 1.0]])
G = LinearOperator.from_matrix(V, D, A, galerkin=False)

m = np.array([1.0, -1.0])
d = G(m)                 # primal vector in D

dp = D.to_dual(d)         # dual element in D'  (a LinearForm)
mp = G.dual(dp)           # dual element in V'
g  = G.adjoint(d)         # primal vector in V

# Consistency check (Riesz-first identity):
# g == V.from_dual( G.dual( D.to_dual(d) ) )
\end{lstlisting}

\paragraph{Type distinctions (what each map consumes).}
The same operator gives rise to three different (typed) actions:
\begin{itemize}
  \item $G$ acts on \emph{primal vectors} $m\in\modelspace$ and returns a primal
    vector $d\in\dataspace$.
  \item $G'\equiv\texttt{G.dual}$ acts on \emph{linear forms} $d'\in\dataspace'$
    and returns a linear form $m'\in\modelspace'$.
  \item $G^*\equiv\texttt{G.adjoint}$ acts on \emph{primal vectors} $d\in\dataspace$
    and returns a primal vector in $\modelspace$.
\end{itemize}
Even in finite dimensions these are not interchangeable: the identifications
between primal and dual spaces are part of the Hilbert-space geometry.

\paragraph{One place where dual and adjoint differ.}
In coordinates, the Banach-dual action is
\begin{equation}
  \label{eq:worked-example-dual-action}
  \text{if } d'\in\dataspace' \text{ has dual components } \mathbf{c}_{d'},
  \qquad
  \Pi_{\modelspace'}\bigl(G'(d')\bigr) = A^\top\,\mathbf{c}_{d'}.
\end{equation}
But the Hilbert adjoint involves the Riesz identification in the data space.
With $\ipD{u}{v}=u^\top W v$ we have, conceptually,
\(\Riesz_{\dataspace}^{-1}(d)\leftrightarrow W\,d\).
Thus the adjoint acts as
\begin{equation}
  \label{eq:worked-example-adjoint-action}
  \Pi_{\modelspace}\bigl(G^*(d)\bigr) = A^\top W\,\Pi_{\dataspace}(d),
\end{equation}
which differs from \eqref{eq:worked-example-dual-action} by the $W$ factor.
This is exactly why the library keeps \texttt{dual} and \texttt{adjoint}
separate: $G'$ is a dual-to-dual map, while $G^*$ is a primal-to-primal map.

\paragraph{The two matrix modes in this example.}
By default, \texttt{G.matrix(...)} returns a matrix-free SciPy
\texttt{LinearOperator} (\texttt{dense=False}). To materialise a dense
array for inspection, use \texttt{dense=True}. For the same operator,
the two dense matrix views are
\begin{align}
  \texttt{G.matrix(dense=True,\, galerkin=False)} &= A
  = \begin{bmatrix} 1 & 2 \\ 0 & 1 \end{bmatrix},\\
  \texttt{G.matrix(dense=True,\, galerkin=True)}  &= W A
  = \begin{bmatrix} 2 & 4 \\ 0 & 3 \end{bmatrix}.
\end{align}
The component view returns the component-to-component map, while the Galerkin
view left-multiplies by the codomain mass/Gram matrix (equivalently,
post-composes with $\Riesz_{\dataspace}^{-1}$) so that the reverse
multiply corresponds to the Hilbert adjoint. This is the same content as the
abstract identity $G^* = \Riesz_{\modelspace} \circ G' \circ \Riesz_{\dataspace}^{-1}$,
...
The class \texttt{LinearOperator} is designed to keep the type distinctions
visible:\;the forward map acts on primal vectors,
the Banach-dual map acts on linear forms, and the Hilbert adjoint acts again on
primal vectors. In code these are exposed as \texttt{G}, \texttt{G.dual}, and
\texttt{G.adjoint}.

\paragraph{The primary constructor.}
The most flexible construction route is
\texttt{LinearOperator(domain, codomain, mapping, ...)}.
The callables you supply are actions on \emph{elements} of the relevant spaces,
not on component vectors.

\begin{codebox}
\texttt{LinearOperator(\
  domain, codomain, mapping,\
  dual\_mapping=..., adjoint\_mapping=...,\
  dual\_base=..., adjoint\_base=...\
)}
\end{codebox}

\paragraph{What the arguments mean.}
Let $G: \modelspace \to \dataspace$ be the intended forward operator.
Then the constructor arguments have the following semantics.
\begin{itemize}
  \item \texttt{mapping}: implements the primal action $G(x)\in\dataspace$ for
    $x\in\modelspace$. This is what runs when you call \texttt{G(x)}.
  \item \texttt{dual\_mapping}: implements the dual action
    $G'(\ell)\in\modelspace'$ for $\ell\in\dataspace'$. In the library, a dual
    element is represented by a \texttt{LinearForm} and is created via
    \texttt{space.to\_dual(...)}. This is what runs when you call
    \texttt{G.dual(yp)}.
  \item \texttt{adjoint\_mapping}: implements the Hilbert adjoint action
    $G^*(y)\in\modelspace$ for $y\in\dataspace$ (primal vector in the codomain).
    This is what runs when you call \texttt{G.adjoint(y)}.
  \item \texttt{dual\_base}, \texttt{adjoint\_base}: internal book-keeping used
    when constructing \texttt{G.dual} and \texttt{G.adjoint}. Most users should
    never set these explicitly.
\end{itemize}

\paragraph{What gets derived if you only provide one mapping.}
The library prefers you to supply at least one of \texttt{dual\_mapping} or
\texttt{adjoint\_mapping}. If you provide only one, the other is derived using
the Riesz maps (i.e. \texttt{to\_dual} and \texttt{from\_dual}) of the spaces,
exactly mirroring the identity
\(G^* = \Riesz_{\modelspace} \circ G' \circ \Riesz_{\dataspace}^{-1}\).

\begin{codebox}
If you provide \texttt{dual\_mapping} (i.e. $G'$), the adjoint is derived as
\texttt{G.adjoint(y) = domain.from\_dual( G.dual( codomain.to\_dual(y) ) )}.

If you provide \texttt{adjoint\_mapping} (i.e. $G^*$), the dual is derived as
\texttt{G.dual(yp) = domain.to\_dual( G.adjoint( codomain.from\_dual(yp) ) )}.
\end{codebox}

If you provide neither, \texttt{G.dual} falls back to the definition
$G'(\ell)=\ell\circ G$ (composition of linear forms with the forward map) and
\texttt{G.adjoint} is then obtained via the Riesz maps. This is mathematically
correct, but can be much slower than a purpose-built adjoint/dual action.

\paragraph{When \texttt{mapping} alone is enough.}
Supplying only \texttt{mapping} is fine when you will \emph{only} evaluate the
forward operator and never need \texttt{G.dual} or \texttt{G.adjoint}
(e.g. a quick diagnostic or a forward simulation). It is also acceptable for
very small problems where the default derived actions are not performance
limiting.

\paragraph{When you should also provide \texttt{dual\_mapping} or \texttt{adjoint\_mapping}.}
Most inverse-problem workloads repeatedly apply an adjoint (or dual): gradients,
Gauss--Newton steps, normal equations, Krylov solvers, and many preconditioners
all rely on \texttt{G.adjoint} or \texttt{G.dual}. In those cases, you should
implement at least one of these actions explicitly.
\begin{itemize}
  \item Prefer supplying \texttt{adjoint\_mapping} when you can write an
    algorithmic reverse operator that maps a data-space vector to a model-space
    vector. This is usually the most direct route for optimisation.
  \item Prefer supplying \texttt{dual\_mapping} when your operator is naturally
    described as a collection of linear functionals (or you already work in
    dual space), so that applying $G'$ is simpler than applying $G^*$.
\end{itemize}

\begin{pitfallbox}
Do not assume ``transpose equals adjoint'' unless your geometry is Euclidean.
In \texttt{pygeoinf}, \texttt{G.dual} (Banach dual) and \texttt{G.adjoint}
(Hilbert adjoint) are different objects in general, related by the Riesz maps
\texttt{to\_dual}/\texttt{from\_dual}.
\end{pitfallbox}

\paragraph{Formal adjoint vs Hilbert adjoint (and why weights matter).}
In many PDE and FEM derivations one first writes a \emph{formal adjoint}
$G^{\dagger}$ with respect to a simple, unweighted $L^2$ pairing. However, the
operator \emph{actually used} by algorithms in the library is the \emph{Hilbert
adjoint} $G^*$ determined by the inner products on $\modelspace$ and
$\dataspace$.
When either space is a \texttt{MassWeightedHilbertSpace}, its inner
product is defined via an underlying space $X$ and a mass operator $M$:
\(\ip{u}{v}{Y}=\ip{M u}{v}{X}\). The Riesz map is modified accordingly (see
\texttt{MassWeightedHilbertSpace.to\_dual} and \texttt{.from\_dual}).

Concretely, if $\modelspace$ and $\dataspace$ are mass-weighted by operators
$M_{\modelspace}$ and $M_{\dataspace}$ (identity in the unweighted case), then
the Hilbert adjoint relates to the formal adjoint by the mass-weighted
conjugation
\begin{equation}
  \label{eq:formal-vs-hilbert-adjoint}
  G^* \,=\, M_{\modelspace}^{-1} \circ G^{\dagger} \circ M_{\dataspace}.
\end{equation}

This is exactly what \texttt{LinearOperator.from\_formal\_adjoint} encodes: you
implement an operator (and its adjoint) on the underlying unweighted spaces,
then \emph{lift} it to the correct weighted geometry so that
\texttt{G.adjoint} matches the Hilbert adjoint dictated by the mass operators.

\begin{codebox}
If you already have an operator on the underlying spaces (``formal'' geometry),
lift it to mass-weighted spaces via
\texttt{LinearOperator.from\_formal\_adjoint(domain, codomain, op\_L2)}.
\end{codebox}

\paragraph{Safe factory routes (recommended when applicable).}
Whenever you have a clean, explicit representation, prefer the dedicated
factories. They avoid ambiguity about whether a matrix is interpreted in
standard or Galerkin form and they return specialized subclasses for better
performance.

\begin{codebox}
\texttt{LinearOperator.from\_matrix(domain, codomain, A, galerkin=False)}
wraps a standard component matrix $A$.

\texttt{LinearOperator.from\_matrix(domain, codomain, K, galerkin=True)}
wraps a Galerkin matrix $K$ (already mass-weighted in the codomain).

\texttt{LinearOperator.self\_adjoint\_from\_matrix(domain, K)}
constructs a self-adjoint operator, interpreting \texttt{K} as Galerkin.

\texttt{LinearOperator.from\_linear\_forms([$\ell_1, \ldots, \ell_N$])}
builds an operator whose rows are linear forms $\ell_i\in\modelspace'$.
\end{codebox}

\section{Advanced routines: randomized spectral surrogates}
\label{sec:linear-operator-advanced-routines}

Beyond construction and basic algebra, \texttt{LinearOperator} provides a small
set of \emph{advanced routines} that build cheap, low-rank spectral surrogates
of an operator. These are primarily intended for \emph{diagnostics} (``what is
the effective rank/spectrum here?''), for quick \emph{surrogate models} in
experiments, and for developing \emph{preconditioning intuition} before you
commit to a full-scale solver.

\paragraph{\texttt{random\_svd}.}
\texttt{G.random\_svd(size\_estimate, ...)} returns three operator factors
\((U,\,\Sigma,\,V)\) with an internal Euclidean space $\mathbb{R}^r$ (where $r$
is the achieved rank returned by the routine) such that
the composition
\(G \approx U\,\Sigma\,V\)
acts as a rank-$r$ approximation of $G$ in the chosen matrix view.
Concretely, $V:\modelspace\to\mathbb{R}^r$ maps a model to low-dimensional
coordinates, $\Sigma$ is a diagonal operator of approximate singular values, and
$U:\mathbb{R}^r\to\dataspace$ maps back into the data space.
Use this when you want a quick picture of dominant input/output directions,
cheaply compress an operator for prototyping, or build a low-rank surrogate that
is fast to apply repeatedly.

\paragraph{\texttt{random\_eig}.}
\texttt{A.random\_eig(size\_estimate, ...)} returns an approximate eigensystem for a square
operator (an automorphism). It produces an ``expansion'' operator
\(U:\mathbb{R}^r\to\modelspace\) together with a diagonal operator of
eigenvalues $\Lambda$ so that, in the intended self-adjoint setting,
\(A \approx U\,\Lambda\,U^*\).
This is useful for sanity-checking spectral behaviour (e.g. identifying dominant
modes, verifying approximate symmetry/self-adjointness in practice), and for
building simple spectral preconditioner sketches.

\paragraph{\texttt{random\_cholesky}.}
\texttt{C.random\_cholesky(size\_estimate, ...)} returns an operator factor
\(F:\mathbb{R}^r\to\modelspace\) intended for positive-definite, self-adjoint
operators such that
\(C \approx F\,F^*\).
It is a convenient way to obtain an approximate ``square root'' of a covariance
or Gram-type operator for exploratory uncertainty quantification, cheap sampling
surrogates, and preconditioning intuition.

\begin{pitfallbox}
These routines are \emph{randomized} and \emph{approximate}. Results depend on
the random seed and on whether the operator is well captured by a low-rank
surrogate.

More subtly, results also depend on \emph{geometry choices}: in \texttt{pygeoinf}
the same operator has different coordinate-level matrix views (component vs
Galerkin; \cref{sec:matrix-representations}). A ``spectrum'' computed in a
component view is not, in general, the Hilbert-space spectrum associated with
your inner product.
Finally, operator properties matter: \texttt{random\_eig} is intended for
self-adjoint/symmetric behaviour in the relevant inner product, and
\texttt{random\_cholesky} assumes positive-definiteness; if these conditions are
violated, the outputs may be misleading even if the code runs.
\end{pitfallbox}

\section{The full commutative diagram}
\label{sec:operator-diagram}

The objects and maps introduced in this chapter — the forward operator $G$,
its Banach dual $G'$, its Hilbert adjoint $\Gadj$, the analysis/synthesis
(coordinate projection/inclusion) maps $\Pi_N$, and the primal and dual
Riesz maps $\Riesz$ — fit together into a single commutative diagram.
Figure~\ref{fig:finite-basis-diagram} shows the full picture: the outer square
lives in the infinite-dimensional spaces $\modelspace$ and $\dataspace$; the
inner rows involve the finite-dimensional surrogates
$\mathbb{R}^N$ and $\mathbb{R}^{N_d}$ used in computations.

The central identity threading the diagram is
\begin{equation}
  \label{eq:adjoint-riesz-commute}
  \Gadj = \RieszM \circ G' \circ \RieszD^{-1},
\end{equation}
which says that the Hilbert adjoint is the Banach dual conjugated by the
Riesz maps of the two spaces.  The blue arrows highlight the maps that
depend on the inner-product geometry of $\modelspace$ and $\dataspace$: any
change in those inner products (e.g.\ using a mass-weighted space) affects
$\RieszM$, $\RieszD$, and therefore $\Gadj$, while leaving $G'$ and the
coordinate maps untouched.

\begin{figure}[h]
\centering
\begin{tikzcd}[row sep=huge, column sep=huge]
\modelspace \arrow[rrrr, "G"]
   \arrow[drr, blue, "\Pi_N" description]
   \arrow[ddd, bend right=25, blue, "\RieszM^{-1}"']
& & & & \dataspace \arrow[dl, blue, "\Pi_{\dataspace}"]
   \arrow[llll, bend right=25, blue, "\Gadj"']
   \arrow[ddd, bend left=25, blue, "\RieszD^{-1}"]\\
& \modelspace_N \arrow[lu, "i_{\modelspace}"] \arrow[d, "\mathcal{R}_{\modelspace_N}"]
& \mathbb{R}^N \arrow[d, bend left=25, "\mathcal{R}_{\mathbb{R}^N}^{-1}"]
   \arrow[l, blue, "\pi_N"']
   \arrow[r, "\Gd"]
& \mathbb{R}^{N_d} \arrow[d, bend left=25, "\mathcal{R}_{\mathbb{R}^{N_d}}^{-1}"]
   \arrow[l, bend right=25, "\Gd^*"']
   \arrow[ru, blue, bend left=25, "\pi_{\dataspace}"]
& \\
& \modelspace_N' \arrow[u, bend left=25, "\mathcal{R}_{\modelspace_N}^{-1}"]
   \arrow[r, "\pi_N'"]
& (\mathbb{R}^N)' \arrow[u, "\mathcal{R}_{\mathbb{R}^N}"]
   \arrow[dll, "\Pi_N'"]
   \arrow[l, bend right=25, "(\pi_N')^{*}"']
& (\mathbb{R}^{N_d})' \arrow[u, "\mathcal{R}_{\mathbb{R}^{N_d}}"]
   \arrow[l, "\Gd'"]
   \arrow[dr, bend right=25, "\Pi_{\dataspace}'"']
& \\
\modelspace' \arrow[uuu, blue, "\RieszM"']
   \arrow[ru, "i'_{\modelspace}"]
& & & &
\dataspace' \arrow[llll, "G'"]
   \arrow[uuu, blue, "\RieszD"]
   \arrow[lu, "\pi_{\dataspace}'"']
\end{tikzcd}
\caption{Summary of all maps between $\modelspace$, $\dataspace$, their duals,
and their finite-$N$ surrogates $\mathbb{R}^N$, $\mathbb{R}^{N_d}$.
Blue arrows are geometry-dependent (they involve the Riesz maps of
$\modelspace$ or $\dataspace$ and hence change when the inner products change).
The key commuting identity is $\Gadj=\RieszM\circ G'\circ \RieszD^{-1}$
(outer square, going top-right $\to$ bottom $\to$ top-left vs top $\to$ top-right).
The inner rows give the finite-dimensional computational surrogates and how they
interlock with the infinite-dimensional picture.}
\label{fig:finite-basis-diagram}
\end{figure}
