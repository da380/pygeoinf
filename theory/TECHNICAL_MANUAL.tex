...
An \emph{orthonormal basis} (ONB) $\{u_j\}_{j=1}^{\infty}$ is a maximal
orthonormal set; every $m \in H$ expands as $m = \sum_j (m, u_j)_H u_j$.
...
\begin{remark}
The quantity $\|\mathbf{c}\|_{\ell^2}$ in \eqref{eq:riesz-bounds} is the standard Euclidean/$\ell^2$ norm on the \emph{coefficient space}. For a general (non-orthonormal) basis, this is \emph{not} the same as the norm of the reconstructed element:
\[
  \|\mathbf{c}\|_{\ell^2} \not= \|\pi(\mathbf{c})\|_H \quad\text{in general.}
\]
If we want coefficient space to carry the \emph{metric induced from $H$ by the synthesis map}, we equip it with the inner product
\[
  (\mathbf{c},\mathbf{d})_\pi := (\pi\,\mathbf{c},\, \pi\,\mathbf{d})_H,
  \qquad \|\mathbf{c}\|_\pi := \sqrt{(\mathbf{c},\mathbf{c})_\pi} = \|\pi\,\mathbf{c}\|_H.
\]
Then \eqref{eq:riesz-bounds} says precisely that $\|\cdot\|_{\ell^2}$ and $\|\cdot\|_\pi$ are equivalent norms. In finite dimensions, the induced inner product becomes
\[
  (\mathbf{a},\mathbf{b})_{\pi_N} := (\pi_N\,\mathbf{a},\,\pi_N\,\mathbf{b})_H = \mathbf{a}^\top\, \mathbf{M}_{\phi,N}\, \mathbf{b},
\]
where $\mathbf{M}_{\phi,N}$ is the Gram matrix from \cref{sec:gram-matrix}.
\end{remark}
...
On the dual space $H'$ the \emph{dual basis} $\{\phi^i\}_{i=1}^\infty \subset H'$
is defined by $\langle\phi^i, \phi_j\rangle = \delta^i_j$. It forms a
Riesz basis for $H'$. The key relationship between the dual basis and the
biorthogonal dual basis is:
\begin{equation}
  \label{eq:dual-basis-riesz-rep}
  \psi_i = \Riesz(\phi^i),
\end{equation}
where $\Riesz : H' \to H$ is the Riesz isomorphism. This equation states that the
biorthogonal dual basis vectors are the Riesz representations of the dual basis
functionals: each $\phi^i \in H'$ maps (via Riesz) to the corresponding
$\psi_i \in H$ that satisfies $(\phi_j, \psi_i)_H = \delta_{ji}$.

To make coefficient representations explicit, let $\{e_j\}$ be the standard basis of $\ell^2$ and $\{e^j\} \subset (\ell^2)'$ the corresponding coordinate functionals, so that
$e^j(\mathbf{c}) = c_j$ and $\langle e^i, e_j\rangle = \delta^i_j$.
For any $\ell' \in (\ell^2)'$ we write
\[
  [\ell']_j := \langle \ell', e_j\rangle = \ell'(e_j).
\]
With this convention, the dual basis induces analysis and synthesis operators on the dual space:
\begin{align}
  \Pi' : H' \to (\ell^2)', \qquad [\Pi'(\ell)]_j &:= \langle \ell, \phi_j\rangle, \\
  \pi' : (\ell^2)' \to H', \qquad \pi'(\ell') &:= \sum_j [\ell']_j \, \phi^j.
\end{align}
These form a pair of bounded inverses on the dual space, analogous to $\Pi$ and $\pi$ on the primal space.
Although $(\ell^2)' \cong \ell^2$ via Riesz for the standard $\ell^2$ inner product, we keep the prime here to avoid silently identifying coefficient vectors and coefficient functionals.
...
\end{equation}
This gives a purely computational recipe: in coefficient space $\ell^2$, the coefficient sequence
$\mathbf{c}$ of $\psi_j$ is $\mathbf{c} = \mathbf{M}_{\phi}^{-1} e_j$, i.e. it solves
\[
  \mathbf{M}_{\phi} \, \mathbf{c} = e_j,
\]
where $e_j$ is the $j$-th standard basis vector of $\ell^2$. In practice one works with an $N$-truncation and solves
$\mathbf{M}_{\phi,N}\,\mathbf{c} = \mathbf{e}_j$ in $\mathbb{R}^N$, then forms $\sum_i c_i \phi_i$.
...
multiplies the vector of inner products $[(\phi_j, m)_H]_j$.
...
On the finite coefficient space $\mathbb{R}^N$, there are (at least) two natural metrics:
the standard Euclidean inner product $\mathbf{a}^\top\mathbf{b}$, and the inner product induced from $H$ by the synthesis map
\[
  (\mathbf{a},\mathbf{b})_{\pi_N} := (\pi_N\,\mathbf{a},\,\pi_N\,\mathbf{b})_H.
\]
With respect to the standard basis $\{\mathbf{e}_j\}$ of $\mathbb{R}^N$, the induced inner product has matrix representation
\begin{equation}
  \label{eq:induced-inner-product-gram}
  (\mathbf{a},\mathbf{b})_{\pi_N} = \mathbf{a}^\top\, \mathbf{M}_{\phi,N}\, \mathbf{b}.
\end{equation}
Equipping $\mathbb{R}^N$ with this induced metric, the Riesz maps are given by the Gram matrix:
\begin{equation}
  \label{eq:riesz-RN}
...
\end{equation}
Here we write elements of $(\mathbb{R}^N)'$ in coordinates using the dual basis $\{\mathbf{e}^j\}$; equivalently, we identify a functional $\ell'\in(\mathbb{R}^N)'$ with its coordinate vector $\mathbf{a}\in\mathbb{R}^N$ defined by $a_j := \ell'(\mathbf{e}_j)$.
...
\begin{itemize}
  \item The \textcolor{blue}{blue horizontal arrows} are the analysis
    ($\Pi_N$) and synthesis ($\pi_N$) operators and their dual maps.
