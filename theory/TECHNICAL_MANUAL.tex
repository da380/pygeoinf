...
For completeness, the analysis and synthesis operators on the dual space are defined
as:
\begin{align}
  \Pi' : H' \to \ell^2, \quad [\Pi'(\ell)]_j &= \langle \ell, \phi_j\rangle, \\
  \pi' : \ell^2 \to H', \quad \pi'(\mathbf{c}) &= \sum_j c_j \phi^j,
\end{align}
and they form a pair of bounded inverses on the dual space, analogous to
$\Pi$ and $\pi$ on the primal space.
