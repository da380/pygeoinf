...
For completeness, the dual basis induces analysis and synthesis operators on the dual space:
\begin{align}
  \Pi' : H' \to (\ell^2)', \quad [\Pi'(\ell)]_j &:= \langle \ell, \phi_j\rangle, \\
  \pi' : (\ell^2)' \to H', \quad \pi'(\ell') &:= \sum_j [\ell']_j \phi^j,
\end{align}
where $[\ell']_j$ denotes the evaluation of the linear functional $\ell' \in (\ell^2)'$ on the
$j$-th standard basis vector of $\ell^2$. These form a pair of bounded inverses on the dual space,
analogous to $\Pi$ and $\pi$ on the primal space.
